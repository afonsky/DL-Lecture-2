...
* Naive implementation: $\hat{y}_i = \frac{\exp(o_i)}{\sum_j \exp(o_j)}$ can overflow/underflow

* **Log-sum-exp trick**: 
$$\log \sum_j \exp(o_j) = \max_j(o_j) + \log \sum_j \exp(o_j - \max_j(o_j))$$

* Why this matters:
  - If $o_i = [1000, 1001, 1002]$, naive softmax → NaN
  - With trick: subtract 1002, compute on $[-2, -1, 0]$ ✓

* Most libraries (PyTorch, TensorFlow) do this automatically
...
#### Starting with: $l(\mathbf{y}, \hat{\mathbf{y}}) = \log \sum\limits_{k=1}^q \exp(o_k) - \sum\limits_{j=1}^q y_j o_j$

Taking derivative w.r.t. $o_j$:
$$\frac{\partial l}{\partial o_j} = \frac{\exp(o_j)}{\sum\limits_{k=1}^q \exp(o_k)} - y_j$$
<br>

**Key insight**: The gradient is simply prediction minus target!
- This is why softmax + cross-entropy is so popular
- Same elegant form as MSE for regression: $\nabla_{\mathbf{w}} \frac{1}{2}(\hat{y} - y)^2 = (\hat{y} - y)\mathbf{x}$
