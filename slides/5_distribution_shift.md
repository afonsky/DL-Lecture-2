...
# Detecting Distribution Shift in Practice

### Statistical Tests:
* **Kolmogorov-Smirnov test**: Compare feature distributions
* **Maximum Mean Discrepancy (MMD)**: Two-sample test in kernel space
* **Population Stability Index (PSI)**: Industry standard for monitoring

<br>

### Monitoring Strategies:
* Track model confidence over time (decreasing → shift likely)
* Monitor feature statistics (mean, variance, quantiles)
...
### When to Retrain:
* Performance drop on validation slice
* Significant PSI change (>0.2 is concerning)
...
```mermaid
graph TD
    A[Detected Distribution Shift] --> B{What Changed?}
    B -->|Input distribution| C[Covariate Shift]
    B -->|Label distribution| D[Label Shift]
    B -->|Label definitions| E[Concept Shift]
    
    C --> F["Importance Weighting<br/>β = p(x) / q(x)"]
    D --> G["Confusion Matrix Method<br/>β = p(y) / q(y)"]
    E --> H[Collect New Labels<br/>Retrain Model]
```
</center>
...
# Note for Covariate Shift
<br>
<center>

```mermaid
graph TD
    F["Importance Weighting<br/>β = p(x) / q(x)"] --> I{Have unlabeled<br/>target data?}
    I -->|Yes| J[Use proposed<br/>algorithm]
    I -->|No| K[Domain adaptation<br/>methods]
```
</center>
